...
class SimpleAutoencoderDetector(AnomalyDetector):
    """Autoencoder-based anomaly detector for bird images.
    
    Args:
        input_size (int): Size of input images (assumed square)
        latent_dim (int): Dimension of latent space
        layer_sizes (List[int]): Hidden layer sizes for encoder/decoder
        ...
        metric (Union[Callable, str]): Metric function or predefined metric name.
                                      If function, signature must be: metric(reconstruction_error: float) -> bool
    """

    def __init__(self, input_size: int, latent_dim: int, layer_sizes: List[int], anomaly_threshold: float, metric: Union[Callable, str]):
    ...
    def predict(self, img: np.ndarray) -> np.ndarray:
        """Predict anomaly scores for input image(s).
        
        Args:
            img: Input image(s) as (h, w, c) or (n_batch, h, w, c) numpy array with values 0-255
            
        Returns:
            Anomaly scores (higher = more anomalous) as numpy array
        """
        # Check input shape and add batch dimension if needed
        if img.ndim == 3:
            # Single image: (h, w, c) -> (1, h, w, c)
            img = img[np.newaxis, ...]
            single_image = True
        elif img.ndim == 4:
            # Batch of images: (n_batch, h, w, c)
            single_image = False
        else:
            raise ValueError(f"Expected 3D (h,w,c) or 4D (n,h,w,c) array, got {img.ndim}D")
        
        # Define transforms to match your dataset preprocessing
        transform = transforms.Compose([
            transforms.ToPILImage(),  # Convert numpy to PIL
            transforms.Resize((self.input_size, self.input_size)),
            transforms.ToTensor(),  # Normalize to [0, 1] and convert to (c, h, w)
        ])
        ...
        for i in range(img.shape[0]):
            ...
            img_tensor = transform(img[i])
            processed_tensors.append(img_tensor)
        
        # Stack into batch tensor: (n_batch, c, h, w)
        batch_tensor = torch.stack(processed_tensors).to(self.device)
        
        # Get reconstruction error
        with torch.no_grad():
            errors = self.autoencoder.get_reconstruction_error(batch_tensor)
        
        # Return to CPU and convert to numpy
        errors = errors.cpu().numpy()
        
        # Return single value if input was single image
        if single_image:
            return errors[0]
        else:
            return errors
